...
// 建立付款資訊卡片 Template Message（未付款/部分付款時使用）
export function createPaymentInfoTemplate(user) {
  ...
  // 確保 text 不超過 160 字元限制
  const textContent = `📚 ${courseName}\n💰 ${coursePrice}\n\n🏦 台灣銀行 (004)\n💳 1234567890123456\n👤 蘇文紹`
  const finalText = textContent.length > 160 ? textContent.substring(0, 157) + '...' : textContent
  ...
  return {
    ...
    template: {
      type: 'buttons',
      // 移除 thumbnailImageUrl，避免 LINE API 400 錯誤
      title: '付款資訊',
      text: finalText,
      actions: [
        {
...
// 建立已付款資訊卡片 Template Message（已付款時使用）
export function createPaidPaymentInfoTemplate(user) {
  const courseName = getCourseName(user.course)
  const coursePrice = getCoursePrice(user.course)
  ...
  // LINE Template Message 的限制：
  // - title: 最多 40 字元
  // - text: 最多 160 字元
  // - label: 最多 20 字元
  // 縮短 text 內容以確保符合限制
  const textContent = `${statusText}\n\n📚 ${courseName}\n💰 ${coursePrice}\n📅 ${paymentDate}\n\n🏦 台灣銀行 (004)\n💳 1234567890123456\n👤 蘇文紹`
  ...
  return {
    ...
    template: {
      type: 'buttons',
      // 移除 thumbnailImageUrl，因為 LINE API 可能對圖片 URL 有嚴格要求
      // thumbnailImageUrl 是可選的，移除後可以避免 400 錯誤
      title: '付款資訊',
      text: finalText,
      actions: [
        {
